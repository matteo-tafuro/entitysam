...
# modified from https://github.com/MinghanLi/UniVS/blob/main/univs/prepare_targets.py
...
from train.utils.comm import box_xyxy_to_cxcywh, convert_box_to_mask

combined_datasets_category_info = {
    'imagenet': (1000, 0),
    'lvis': (1203, 1000),
    'burst': (1203, 1000),
    'ytvis21': (40, 2203),
    ...
    'viposeg': (124, 2924),
    'ytvis19': (40, 3048),
    'entityseg_instance': (206, 3088),
    'entityseg_panoptic': (644, 3294),
}
...
class PrepareTargets:
    def __init__(
        ...
        print(f"PrepareTargets initialized with device: {device}")
        if clip_class_embed_path:
            self.clip_cls_text_emb = torch.load(clip_class_embed_path,map_location=device)
        else:
            self.clip_cls_text_emb = None
        ...
        for k, (gt_instances, targets_per_video) in enumerate(zip(clip_gt_instances, targets)):
            ...
            if targets_per_video["task"] == "grounding":
                ...
                if is_train and targets_per_video["prompt_type"] == 'text':
                    ...
                    # Move prompt_gt_labels to the same device as clip_cls_text_emb before indexing
                    gt_instances["prompt_gt_labels"] = gt_instances["prompt_gt_labels"].to(clip_cls_text_emb.device)
