...
# modified from https://github.com/facebookresearch/sam2/blob/main/sam2/sam2_video_predictor.py
...
class SAM2CameraQueryIoUPredictor(SAM2QueryIoUBase):
    ...
    def prepare_data(
        ...
        if isinstance(img, np.ndarray):
            ...
            img_np = (
                cv2.resize(
                    img_np, (image_size, image_size), interpolation=cv2.INTER_CUBIC
                )
                / 255.0
            )
        ...
        img -= img_mean
        img /= img_std
        return img.to(self.device), width, height
    ...
    #     obj_idx = self._obj_id_to_idx(self.condition_state, obj_id)
    ...
    #         self.condition_state=self.condition_state,
    ...
    #         self.condition_state, consolidated_out["pred_masks_video_res"]
    #     )
    ...
    # @torch.inference_mode()
    # def propagate_in_video_preflight(self, self.condition_state):
    ...
    #     batch_size = self._get_obj_num(self.condition_state)
    ...
    #             consolidated_out = self._consolidate_temp_output_across_obj(
    #                 self.condition_state, frame_idx, is_cond=is_cond, run_mem_encoder=True
    #             )
    #             # merge them into "output_dict" and also create per-object slices
    #             output_dict[storage_key][frame_idx] = consolidated_out
    #             self._add_output_per_object(
    #                 self.condition_state, frame_idx, consolidated_out, storage_key
    #             )
    ...
    #                 self._clear_non_cond_mem_around_input(self.condition_state, frame_idx)
    ...
    def track(
        ...
        output_dict = self.condition_state["output_dict"]
        obj_ids = self.condition_state["obj_ids"]
        batch_size = self.sam_mask_decoder.num_queries
    ...
    def propagate_in_video(
        ...
        if reverse and progress_order is None:
            end_frame_idx = max(start_frame_idx - max_frame_num_to_track, 0)
            if start_frame_idx > 0:
                processing_order = range(start_frame_idx, end_frame_idx - 1, -1)
            else:
                processing_order = []  # skip reverse tracking if starting from frame 0
        elif progress_order is None:
            end_frame_idx = min(
                start_frame_idx + max_frame_num_to_track, num_frames - 1
            )
            processing_order = range(start_frame_idx, end_frame_idx + 1)
        else:
            processing_order = progress_order
        ...
        for frame_idx in tqdm(processing_order, desc="propagate in video"):
            ...
            # _, video_res_masks = self._get_orig_video_res_output(
            #     self.condition_state, pred_masks
            # )
    ...
    @torch.inference_mode()
    def clear_all_prompts_in_frame(self, frame_idx, obj_id, need_output=True):
        """Remove all input points or mask in a specific frame for a given object."""
        obj_idx = self._obj_id_to_idx(obj_id)

        # Clear the conditioning information on the given frame
        self.condition_state["point_inputs_per_obj"][obj_idx].pop(frame_idx, None)
        self.condition_state["mask_inputs_per_obj"][obj_idx].pop(frame_idx, None)
        ...
        for obj_idx2 in range(batch_size):
            if frame_idx in self.condition_state["point_inputs_per_obj"][obj_idx2]:
                frame_has_input = True
                break
            if frame_idx in self.condition_state["mask_inputs_per_obj"][obj_idx2]:
                frame_has_input = True
                break
        ...
        if not frame_has_input:
            ...
            if out is not None:
                ...
                self.condition_state["frames_already_tracked"].pop(frame_idx, None)
            # Similarly, do it for the sliced output on each object.
            for obj_idx2 in range(batch_size):
                obj_output_dict = self.condition_state["output_dict_per_obj"][obj_idx2]
    ...
    def _run_single_frame_inference(
        ...
        iou_pred = current_out["iou_pred"]
        cls_logits_pred = current_out["cls_logits_pred"]
        ...
        return compact_current_out, pred_masks_gpu, iou_pred, cls_logits_pred
    ...
    # @torch.inference_mode()
    # def remove_object(self, self.condition_state, obj_id, strict=False, need_output=True):
    #     """
    #     Remove an object id from the tracking state. If strict is True, we check whether
    #     the object id actually exists and raise an error if it doesn't exist.
    #     """
    #     old_obj_idx_to_rm = self.condition_state["obj_id_to_idx"].get(obj_id, None)
    #     updated_frames = []
    #     # Check whether this object_id to remove actually exists and possibly raise an error.
    #     if old_obj_idx_to_rm is None:
    #         if not strict:
    #             return self.condition_state["obj_ids"], updated_frames
    #         raise RuntimeError(
    ...
    #     if len(self.condition_state["obj_id_to_idx"]) == 1:
    #         self.reset_state(self.condition_state)
    #         return self.condition_state["obj_ids"], updated_frames
    ...
    #         self.condition_state["point_inputs_per_obj"][old_obj_idx_to_rm]
    #     )
    #     obj_input_frames_inds.update(
    #         self.condition_state["mask_inputs_per_obj"][old_obj_idx_to_rm]
    #     )
    #     for frame_idx in obj_input_frames_inds:
    #         self.clear_all_prompts_in_frame(
    #             self.condition_state, frame_idx, obj_id, need_output=False
    #         )

    #     # Step 1: Update the object id mapping (note that it must be done after Step 0,
    #     # since Step 0 still requires the old object id mappings in self.condition_state)
    #     old_obj_ids = self.condition_state["obj_ids"]
    ...
    #     self.condition_state["obj_id_to_idx"] = dict(zip(new_obj_ids, new_obj_inds))
    #     self.condition_state["obj_idx_to_id"] = dict(zip(new_obj_inds, new_obj_ids))
    #     self.condition_state["obj_ids"] = new_obj_ids
    ...
    #     _map_keys(self.condition_state["point_inputs_per_obj"])
    #     _map_keys(self.condition_state["mask_inputs_per_obj"])
    #     _map_keys(self.condition_state["output_dict_per_obj"])
    #     _map_keys(self.condition_state["temp_output_dict_per_obj"])
    ...
    #             ]
    #             # "maskmem_pos_enc" is the same across frames, so we only need to store one copy of it
    #             out["maskmem_pos_enc"] = self._get_maskmem_pos_enc(self.condition_state, out)
    ...
    #             self._add_output_per_object(
    #                 self.condition_state, frame_idx, out, storage_key
    #             )

    #     _slice_state(self.condition_state["output_dict"], "cond_frame_outputs")
    #     _slice_state(self.condition_state["output_dict"], "non_cond_frame_outputs")
    ...
    def init_state_train(
        ...
        self.condition_state = {}
        self.condition_state["images"] = images
        self.condition_state["num_frames"] = len(images)
        # whether to offload the video frames to CPU memory
        # turning on this option saves the GPU memory with only a very small overhead
        self.condition_state["offload_video_to_cpu"] = offload_video_to_cpu
        # whether to offload the inference state to CPU memory
        ...
        self.condition_state["video_height"] = video_height
        self.condition_state["video_width"] = video_width
    ...
    def forward(self, batched_inputs):
        ...
        assert len(batched_inputs) == 1, "Only support one video propagation"
        images = []
        video = batched_inputs[0]
        ...
        train_state = self.init_state_train(images, video_height, video_width)
        self.reset_state_train(train_state)
